...
// reader global untuk membaca input dari stdin (nama_anggota)
...
func InputString(prompt string) string {
	...
	// menghapus karakter newline dan spasi di awal/akhir (nama_anggota)
	return strings.TrimSpace(input)
}

// membaca input integer dari pengguna dengan validasi tipe data (nama_anggota)
func InputInt(prompt string) int {
	// melakukan perulangan sampai input yang valid diterima (nama_anggota)
	for {
		fmt.Print(prompt)
		input, _ := reader.ReadString('\n')
...
func InputFloat(prompt string) float64 {
	// melakukan perulangan sampai input angka positif yang valid diterima (nama_anggota)
	for {
		fmt.Print(prompt)
		input, _ := reader.ReadString('\n')
		input = strings.TrimSpace(input)
...
func ValidasiTanggal(tanggal string) bool {
	// memeriksa panjang string dan pemisah yang benar (nama_anggota)
	if len(tanggal) != 10 {
		return false
	}
	...
	// memeriksa rentang nilai bulan dan hari (nama_anggota)
...
func InputTanggal(prompt string) string {
	// mengulang permintaan input hingga format tanggal yang benar diterima (nama_anggota)
	for {
