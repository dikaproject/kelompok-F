...
// mencetak baris garis horizontal dengan panjang yang ditentukan (nama_anggota)
func CetakGaris(panjang int) {
	fmt.Println(strings.Repeat("=", panjang))
}

// mencetak garis pemisah dengan karakter minus (nama_anggota)
func CetakPemisah(panjang int) {
	fmt.Println(strings.Repeat("-", panjang))
}

// mencetak header utama aplikasi Waste-Track (nama_anggota)
func CetakHeader() {
	fmt.Println()
	CetakGaris(44)
...
// mencetak tabel seluruh data warga beserta total berat sampah mereka (nama_anggota)
func TampilTabelWarga() {
	...
	CetakPemisah(76)
	// mencetak setiap baris data warga beserta total berat akumulasinya (nama_anggota)
...
// mencetak tabel seluruh log setoran yang pernah dicatat (nama_anggota)
func TampilTabelSetoran() {
	...
	CetakPemisah(72)
	// mencetak setiap entri log setoran dengan nama warga dan jenis sampah (nama_anggota)
...
// mencetak riwayat setoran sampah untuk satu warga tertentu (nama_anggota)
func TampilSetoranByWarga(wargaID int) {
	...
	CetakPemisah(42)
	// mencetak setiap entri setoran milik warga yang dicari (nama_anggota)
	for i := 0; i < count; i++ {
		s := setoran[i]
...
// menghasilkan bar chart ASCII berdasarkan nilai relatif terhadap nilai maksimum (nama_anggota)
func buatBar(nilai, maks float64, panjangMaks int) string {
	if maks == 0 {
		return ""
	}
	// menghitung panjang bar secara proporsional (nama_anggota)
	panjang := int((nilai / maks) * float64(panjangMaks))
	return strings.Repeat("█", panjang)
}

// mencetak statistik akumulasi setoran sampah berdasarkan nomor minggu (nama_anggota)
func TampilStatistikMingguan(minggu int) {
	...
	// akumulasi total berat per jenis sampah dari setoran minggu ini (nama_anggota)
	...
	// mencari nilai terbesar sebagai acuan skala bar chart (nama_anggota)
	maxBerat := 0.0
	for j := 0; j < data.JumlahJenis; j++ {
	...
	// mencetak tabel statistik per jenis sampah beserta bar chart (nama_anggota)
	...
	// akumulasi total berat per warga untuk minggu ini (nama_anggota)
	...
	for i := 0; i < count; i++ {
		...
		found := false
		// mencari apakah warga sudah ada di daftar akumulasi (nama_anggota)
		for k := 0; k < nWarga; k++ {
	...
	// mengurutkan warga berdasarkan total berat descending menggunakan selection sort (nama_anggota)
	...
	// menampilkan daftar warga pengumpul terbanyak minggu ini (nama_anggota)
	tampil := nWarga
	if tampil > 3 {
		tampil = 3
	}
